...
def sample_n_tokens(score_path, out_path, n, T, n_processes=CPU_COUNT):
    ...
    """
    global dist, scores, softmax_dist, selected_tokens, sample_size

    n //= 512 # 512 tokens per sample
    sample_size = int(1e6) # How many samples each call to _sample draws
    ...
    with multiprocessing.Pool(processes=n_processes) as pool:
        chunksize = 1
        with tqdm(desc='Sampling', unit='Chunks', colour='green', total=n//sample_size//chunksize) as pbar:
            for result in pool.imap_unordered(_sample, range(n//sample_size), chunksize=chunksize):
                selected_tokens += result
                pbar.update(1)
...
def _sample(_):
        sampled_score = np.random.choice(scores, size=sample_size, p=softmax_dist)
...
if __name__ == '__main__':
    ...
    os.makedirs(OUT_PATH) # No exist_ok to safeguard overwriting selections
    print(f'Writing with mode {mode} to {OUT_PATH}')
    if mode[0] == 'sample':
