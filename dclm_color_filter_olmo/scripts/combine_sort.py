...
def save_jsonl(data, output_path):
    ...
    with open(f"{output_path}/chunk_scores.jsonl", 'w') as f:
        for item in data:
...
def subtract_scores(folder1_data, folder2_data):
    ...
    result = sorted(result, key=lambda x: x["score"][0], reverse=True)
    return result
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(cfg['checkpoints_path'], "combined")
